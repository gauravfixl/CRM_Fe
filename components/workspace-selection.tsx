...
import { getMyWorkspaces, createWorkspace, deleteWorkspace } from "@/hooks/workspaceHooks"
...
export function WorkspaceSelection() {
  ...
  const handleCreateWorkspace = async () => {
    ...
    try {
      ...
      dispatch({ type: "CREATE_WORKSPACE", payload: newWorkspace.workspace })
      setIsCreateOpen(false)
      setWorkspaceName("")
      setWorkspaceDescription("")
      router.push(`/${orgName}/modules/workspaces/${newWorkspace.workspace._id}/dashboard`)
    } catch (err: any) {
        ...
        {filteredWorkspaces.length === 0 ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {paginatedGridWorkspaces.map((workspace) => {
              const projectCount = state.projects.filter((p) => p.workspaceId === workspace.id).length
              return (
