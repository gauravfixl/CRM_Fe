...
import { addTeamMember, deleteTeamMember, getAllTeamMembers } from "@/hooks/teamHooks"
...
export default function People({ entityId, parentId, level }: PeopleProps) {
  ...
  const [deleteLevel, setDeleteLevel] = useState<"workspace" | "project" | "team">("workspace")
  ...
  const api = {
    add: async (payload: any) => {
      if (level === "project") return addProjectMembers(entityId, payload)
    ...
    delete: async (memberId: string, payload?: any) => {
      if (level === "workspace") return deleteWorkspaceMember(entityId, payload)
      if (level === "project") return deleteProjectMember(entityId, payload)
      if (level === "team" && parentId) return deleteTeamMember(entityId, memberId, parentId)
      return null
    },
    getAll: async (id?: string, l?: string) => {
      if (l === "project") return getProjectMembers(id || entityId)
      if (l === "workspace") return getWorkspaceMembers(id || entityId)
      if (l === "team") return getAllTeamMembers(id || entityId, parentId)
      return null
    },
  }
  ...
  const fetchMembers = async () => {
    try {
      ...
      if (level === "workspace" && parentId) {
        const orgRes = await fetchUsersApi(parentId)
  ...
  const handleDeleteClick = (member: Member, lvl: "workspace" | "project" | "team") => {
  ...
  const handleDeleteMember = async (member: Member, reason: string, level: "workspace" | "project" | "team") => {
    try {
      showLoader()
      let res;
      if (level === "workspace") {
        res = await deleteWorkspaceMember(entityId, { email: member.email, reason: reason || "Removed by administrator" });
      } else if (level === "project") {
        res = await deleteProjectMember(entityId, { memberId: member.id });
      } else if (level === "team" && parentId) {
        res = await deleteTeamMember(entityId, member.id, parentId);
      }
