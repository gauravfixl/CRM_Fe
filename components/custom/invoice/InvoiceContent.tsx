...
export function InvoiceContent({ data }: InvoiceContentProps) {
  ...
  return (
    <div className="max-w-2xl p-4 rounded-xl bg-white shadow-sm">
      {/* Header */}
      ...
      <div className="flex justify-between border rounded-md px-1 text-xs text-gray-600 bg-gray-50">
        <p>Invoice Date: {data?.invoiceDate ?? "Invalid Date"}</p>
        <p>Due Date: {data?.dueDate ?? "Invalid Date"}</p>
      </div>
