...
export default function ClientGovernancePage() {
    ...
    const quickAccessItems = [
        { label: "Account Attribution", path: `/${params.orgName}/modules/settings/entitlements/clients/ownership` },
        { label: "Internal Transfers", path: `/${params.orgName}/modules/settings/entitlements/clients/ownership` },
        { label: "Privacy Scrubbing", path: `/${params.orgName}/modules/settings/entitlements/clients/privacy` },
        { label: "Visibility Tiers", path: `/${params.orgName}/modules/settings/entitlements/clients/visibility` },
    ]
        ...
        <div className="flex flex-col h-full w-full bg-slate-50/50 p-6 space-y-6 overflow-y-auto font-sans">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-6 border border-slate-200 rounded-xl shadow-sm">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-gradient-to-r from-primary/70 to-primary rounded-xl flex items-center justify-center text-white shadow-sm">
                        <Users className="w-6 h-6" />
                    </div>
                    <div>
                        <h1 className="text-sm font-semibold text-slate-900 leading-none">Client Governance</h1>
                        <p className="text-xs text-slate-500 mt-1 font-medium">Govern the institutional master schemes for client accounts, lifecycles, and legal data retention.</p>
                    </div>
                </div>
                <div className="flex gap-2">
                    <Button
                        variant="outline"
                        className={`h-8 rounded-lg border-slate-200 text-xs font-medium px-4 shadow-sm hover:bg-slate-50 ${isSyncing ? 'animate-pulse' : ''}`}
                        onClick={handleSync}
                        disabled={isSyncing}
                    >
                    ...
                    <Button
                        className="h-8 rounded-lg text-xs font-medium bg-blue-600 hover:bg-blue-700"
                        onClick={() => router.push(`/${params.orgName}/modules/settings/entitlements/clients/ownership`)}
                    >
                        Ownership Map
                    </Button>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <SmallCard className="bg-gradient-to-r from-primary/70 to-primary border-none rounded-xl shadow-sm text-white">
                    ...
                    <SmallCardContent className="text-left px-4 pb-4">
                        <p className="text-xl font-semibold text-white">2,841</p>
                ...
                <SmallCard className="bg-white border-slate-200 rounded-xl shadow-sm">
                    ...
                    <SmallCardContent className="text-left px-4 pb-4">
                        ...
                        <p className="text-[10px] text-emerald-600 font-medium mt-1">Compliant</p>
                    </SmallCardContent>
                </SmallCard>
                ...
                <SmallCard className="bg-white border-slate-200 rounded-xl shadow-sm">
                    ...
                    <SmallCardContent className="text-left px-4 pb-4">
                        <p className="text-xl font-semibold text-slate-900">03</p>
                        <p className="text-[10px] text-rose-500 font-medium mt-1">Requires Action</p>
                    </SmallCardContent>
                </SmallCard>
                ...
                <SmallCard className="bg-white border-slate-200 rounded-xl shadow-sm">
                    ...
                    <SmallCardContent className="text-left px-4 pb-4">
                        ...
                        <p className="text-[10px] text-slate-400 font-medium mt-1">High-Value Protected</p>
                    </SmallCardContent>
                </SmallCard>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-8 bg-white border border-slate-200 rounded-xl shadow-sm h-full flex flex-col">
                    <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                        <div>
                            <h3 className="text-sm font-semibold text-slate-900">Data Retention & Legal Schemes</h3>
                            <p className="text-[11px] text-slate-500">Institutional standards for data lifecycle and compliance purges.</p>
                        </div>
                    </div>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="bg-slate-50/50 hover:bg-slate-50/50">
                                    <TableHead className="text-[10px] font-medium text-slate-500 px-5">Data Category</TableHead>
                                    <TableHead className="text-[10px] font-medium text-slate-500">Retention Period</TableHead>
                                    <TableHead className="text-[10px] font-medium text-slate-500">Governance Status</TableHead>
                            ...
                            <TableBody>
                                {retentionPolicies.map((item, idx) => (
                                    <TableRow key={idx} className="hover:bg-slate-50/50 transition-colors border-b border-slate-100 last:border-0">
                                        ...
                                        <TableCell>
                                            <p className="text-[11px] text-slate-600 font-medium">{item.retention}</p>
                                        </TableCell>
                                        <TableCell>
                                            <div className="flex items-center gap-2">
                                                <Badge className={`rounded-md text-[10px] font-medium px-2 leading-tight ${item.status === 'Enforced' ? 'bg-blue-50 text-blue-600 border-none' :
                    ...
                    <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
                        ...
                        <div className="space-y-2">
                            {quickAccessItems.map((item) => (
                                <div
                                    key={item.label}
                                    className="flex items-center justify-between p-2.5 border border-slate-50 rounded-lg hover:bg-slate-50 cursor-pointer group transition-all"
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
                <div className="flex items-center gap-3 mb-6 border-b border-slate-50 pb-4">
                    ...
                    <h3 className="text-sm font-semibold text-slate-900">Institutional Database Integrity</h3>
                </div>
