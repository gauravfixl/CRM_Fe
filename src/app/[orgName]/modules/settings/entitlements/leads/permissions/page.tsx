        ...
        <div className="flex flex-col gap-6 p-6 min-h-screen bg-[#fafafa]">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <h1 className="text-xl font-semibold text-gray-900">Baseline Permissions</h1>
                            <Badge className="bg-zinc-100 text-zinc-500 hover:bg-zinc-100 border-none text-[9px] font-medium">Access Control</Badge>
                        </div>
                        <p className="text-xs text-gray-600 font-medium">Define foundational access roles and capabilities for Lead Management management.</p>
                    </div>
                </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <SmallCard className="border bg-gradient-to-r from-primary/70 to-primary text-white shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                    <SmallCardContent className="p-4 flex flex-row items-center justify-between">
                        <div>
                            <p className="text-white text-xs opacity-80">Access Model</p>
                            <p className="text-white text-xl font-semibold">RBAC</p>
                            <p className="text-[10px] text-white opacity-80">Role-Based Access Logic</p>
                        </div>
                ...
                <SmallCard className="border bg-white shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                    <SmallCardContent className="p-4 flex flex-row items-center justify-between">
                        <div>
                            <p className="text-xs text-gray-600">Data Visibility</p>
                            <p className="text-xl font-semibold text-gray-900">Restricted</p>
                            <p className="text-[10px] text-gray-500">Owner-only editing active</p>
                        </div>
                ...
                <SmallCard className="border bg-white shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                    <SmallCardContent className="p-4 flex flex-row items-center justify-between">
                        <div>
                            <p className="text-xs text-gray-600">Override Mode</p>
                            <p className="text-xl font-semibold text-rose-600">Off</p>
                            <p className="text-[10px] text-rose-500">Firms cannot override</p>
                        </div>
            ...
            <div className="bg-white rounded-xl border border-zinc-200 shadow-sm overflow-hidden mt-2">
                <div className="p-6 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/20">
                    <h3 className="text-base font-medium text-gray-900">Default Role Permission Matrix</h3>
                    <Button variant="outline" className="h-8 rounded-lg text-xs font-medium bg-white shadow-sm" onClick={() => handleAction("Permission configuration opened")}>
                ...
                <Table>
                    <TableHeader className="bg-zinc-50/50">
                        <TableRow className="hover:bg-transparent border-b-zinc-100">
                            <TableHead className="py-4 px-6 text-[11px] font-medium text-gray-500">Base Role</TableHead>
                            <TableHead className="py-4 text-[11px] font-medium text-gray-500 text-center">Create</TableHead>
                            <TableHead className="py-4 text-[11px] font-medium text-gray-500 text-center">Edit</TableHead>
                            <TableHead className="py-4 text-[11px] font-medium text-gray-500 text-center">Delete</TableHead>
                            <TableHead className="py-4 text-[11px] font-medium text-gray-500 text-center">Convert</TableHead>
                            <TableHead className="py-4 text-[11px] font-medium text-gray-500 text-center">View All</TableHead>
                        ...
                        {permissionMatrix.map((item, idx) => (
                            <TableRow key={idx} className="hover:bg-zinc-50/50 transition-colors group">
                                <TableCell className="py-4 px-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            <span className="text-sm font-medium text-gray-900 group-hover:text-blue-600 transition-colors">{item.role}</span>
                                            <span className="text-[9px] font-medium text-gray-400">{item.type} Defined</span>
                                        </div>
                                    </div>
                                </TableCell>
                                ...
                                <TableCell className="py-4 text-right pr-6">
                                    <Button size="sm" variant="ghost" disabled={item.type === 'System'} className="h-8 w-8 p-0 rounded-lg hover:bg-zinc-100 text-zinc-300 transition-all" onClick={() => item.type !== 'System' && handleAction("Role editor opened")}>
            ...
            <div className="bg-white rounded-xl border border-zinc-200 p-8 shadow-sm">
                <div className="flex items-center gap-4 mb-8 border-b border-zinc-100 pb-6">
                    ...
                    <div>
                        <h3 className="text-base font-medium text-gray-900">Global Sovereignty Policy</h3>
                        <p className="text-[11px] text-gray-500 font-medium">Control whether child firms can define their own lead permission matrices.</p>
                    </div>
                </div>
                <div className="flex flex-col md:flex-row items-center justify-between gap-6 p-6 bg-zinc-50/50 rounded-xl border border-zinc-100 group transition-all hover:bg-white hover:border-blue-100">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                            <span className="text-xs font-medium text-gray-900">Firm-Level Authority Override</span>
                            <span className="text-[10px] text-gray-500 font-medium mt-1">Status: Restricted by Org Admin</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-4">
                        <span className="text-[10px] font-medium text-gray-500">Enable Firm Overrides?</span>
...
function PermissionToggle({ active }: { active: boolean | string }) {
    ...
    if (active === "OWN") return <div className="flex items-center justify-center"><Badge className="bg-blue-50 text-blue-600 hover:bg-blue-50 border-none text-[8px] font-medium px-2 h-4">Owner Only</Badge></div>
    return null
}
