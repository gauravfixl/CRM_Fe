...
export default function LicensesPage() {
    ...
    const errors = useMemo(() => {
        ...
        const duplicate = licenseUsers.find(
            (u) =>
        ...
        if (touched.name) {
            ...
            else if (!/^[A-Za-z\s.'-]+$/.test(name)) e.name = "Only letters, spaces, and . ' - allowed";
            else if (name.length < 2) e.name = "Name must be at least 2 characters";
            else if (name.length > 80) e.name = "Name is too long";
        }
        if (touched.email) {
            if (!email) e.email = "Email is required";
            else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) e.email = "Enter a valid email (e.g. user@company.com)";
            else if (duplicate) e.email = "This email already has a license";
        }
        return e;
    }, [formName, formEmail, touched, licenseUsers, editingUser]);
    ...
    const validateAll = () => {
        ...
        if (duplicate) return "This email already has a license";
        return null;
    };
    ...
    const cycleRoleFilter = () => setRoleFilterIndex((p) => (p + 1) % ROLE_FILTERS.length);
    const cycleStatusFilter = () => setStatusFilterIndex((p) => (p + 1) % STATUS_FILTERS.length);
    ...
    const renderFormFields = () => (
        <div className="space-y-4">
            <Field label="Full Name" required error={errors.name} hint="Only letters, spaces, and . ' -">
                <Input
            ...
            <Field label="Email" required error={errors.email} hint="Used to send the invite">
                <Input
                    type="email"
            ...
            <Field label="Role" required hint="Determines access permissions">
            ...
            <Field label="Plan" hint="Inherited from your organization">
                <Input
    ...
    return (
        <div className="flex flex-col min-h-screen bg-transparent">
                ...
                <div className="bg-white border border-zinc-200 rounded-none shadow-lg overflow-hidden">
                    <div className="p-4 border-b border-zinc-100 flex flex-col md:flex-row gap-4 justify-between items-center bg-zinc-50/50">
                        ...
                        <div className="flex gap-2 w-full md:w-auto">
                            <Button variant="outline" className="rounded-none border-zinc-200 text-xs h-9 gap-2 flex-1 md:flex-none font-medium bg-white" onClick={cycleRoleFilter}>
                                <Filter size={14} /> {activeRoleFilter === "All" ? "Filter Roles" : activeRoleFilter}
                            </Button>
                            <Button variant="outline" className="rounded-none border-zinc-200 text-xs h-9 gap-2 flex-1 md:flex-none font-medium bg-white" onClick={cycleStatusFilter}>
                            ...
                            <tbody className="divide-y divide-zinc-100">
                                {filteredUsers.map((user) => (
                                    <tr key={user.id} className="hover:bg-primary/5 transition-colors group">
                                        ...
                                        <td className="px-6 py-3">
                                            <Badge className="bg-zinc-900 text-white border-none rounded-none px-2 py-0.5 text-[10px] font-medium">{user.plan}</Badge>
                                        </td>
                                        <td className="px-6 py-3 text-xs text-gray-600 font-medium">{user.assignedDate}</td>
                                        <td className="px-6 py-3">
                                        ...
                                        <td className="px-6 py-3 text-right">
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end" className="rounded-none border-zinc-200 shadow-lg p-2 min-w-[160px]">
                                                    ...
                                                    <DropdownMenuItem className="text-xs font-medium p-2 rounded-md cursor-pointer" onClick={() => toggleUserStatus(user.id)}>
                                                        {user.status === "Suspended" ? "Restore License" : "Suspend Access"}
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem className="text-xs font-medium p-2 rounded-md cursor-pointer" onClick={() => openEditModal(user)}>
                                                        Reassign
                                                    </DropdownMenuItem>
                                                    <DropdownMenuSeparator className="my-1" />
                                                    <DropdownMenuItem className="text-xs font-medium p-2 text-rose-600 rounded-md cursor-pointer flex items-center gap-2" onClick={() => removeUser(user.id)}>
            ...
            <SideFormSheet
                ...
                description="Add a new user to your Enterprise Pro plan."
            ...
            <SideFormSheet
                ...
                description="Update the license assignment details."
