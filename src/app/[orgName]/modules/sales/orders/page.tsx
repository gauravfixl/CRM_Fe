    ...
    const [orders, setOrders] = useState([
        {
            ...
            amount: "$152,400.00",
            status: "Processing",
            delivery: "18 Jan 2026",
            items: "12 SKUs",
            fulfillment: "Warehouse-A",
            owner: "Sarah J.",
            color: "text-blue-600",
            bg: "bg-blue-50"
        },
        {
            ...
            amount: "$45,000.00",
            status: "Shipped",
            delivery: "16 Jan 2026",
            items: "5 SKUs",
            fulfillment: "Cloud Node-1",
            owner: "Bob S.",
        ...
        },
        {
            ...
            amount: "$12,000.00",
            status: "Pending",
            delivery: "20 Jan 2026",
            items: "2 SKUs",
            fulfillment: "Warehouse-B",
            owner: "Alice P.",
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#F8F9FC] dark:bg-black text-zinc-900 dark:text-zinc-100 transition-colors">
            ...
            <div className="flex-1 p-8 space-y-12 max-w-[1750px] mx-auto w-full">
                {/* Visual Fulfillment HUD */}
                <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                    {[
                        { label: "Pending Fulfillment", value: "24", icon: Box, color: "text-blue-600", trend: "+2 Volume" },
                        { label: "Avg Fulfillment", value: "8.2h", icon: Clock, color: "text-emerald-600", trend: "-1.2h Speed" },
                        { label: "Order Velocity", value: "High", icon: Activity, color: "text-amber-600", trend: "Elite Flow" },
                        { label: "Global Coverage", value: "94%", icon: Truck, color: "text-purple-600", trend: "Maximum" },
                    ].map((stat, i) => (
                ...
                {/* Main Order Ledger Table Control */}
                <div className="bg-white dark:bg-zinc-900 rounded-[64px] border border-zinc-200 dark:border-zinc-800 shadow-3xl flex flex-col transition-all overflow-hidden lg:min-h-[600px]">
                    <div className="p-14 border-b border-zinc-100 dark:border-zinc-800 flex flex-col md:flex-row md:items-center justify-between gap-10 bg-zinc-50/20 dark:bg-zinc-800/20">
                        <div className="flex items-center gap-10">
                            ...
                            <div>
                                <h3 className="text-4xl font-black uppercase tracking-tighter italic leading-none text-zinc-900 dark:text-white uppercase italic">Execution Node Ledger</h3>
                                <p className="text-sm font-black text-zinc-400 mt-3 uppercase tracking-widest italic opacity-70">Unified management of sales orders and fulfillment cycles.</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-4">
                            <div className="relative w-96">
                                <Search className="absolute left-5 top-1/2 -translate-y-1/2 h-5 w-5 text-zinc-400" />
                                <CustomInput placeholder="Search orders, clients, or fulfillment nodes..." className="pl-14 h-14 rounded-[28px] bg-white border-zinc-200 shadow-inner font-bold italic" />
                            </div>
                    ...
                    <div className="p-10 space-y-8">
                            ...
                            {orders.map((order, idx) => (
                                ...
                                >
                                    <div className="flex items-center gap-10 w-full xl:w-auto">
                                        <div className={`h-24 w-24 rounded-[36px] ${order.bg} ${order.color} flex items-center justify-center shadow-inner relative overflow-hidden group-hover:scale-105 transition-transform duration-700`}>
                                            <span className="text-3xl font-black italic">{order.client.charAt(0)}</span>
                                        </div>
                                        <div className="space-y-3">
                                            <div className="flex items-center gap-4">
                                                <Badge className={`px-4 py-1.5 rounded-full font-black text-[9px] tracking-widest border-none ${order.status === 'Delivered' ? 'bg-emerald-100 text-emerald-700 shadow-[0_0_15px_rgba(16,185,129,0.2)]' :
                                                        order.status === 'Shipped' ? 'bg-blue-100 text-blue-700' :
                                                            order.status === 'Processing' ? 'bg-amber-100 text-amber-700' : 'bg-zinc-100 text-zinc-700'
                                                    } uppercase italic`}>{order.status}</Badge>
                                                <span className="text-[10px] font-black uppercase text-zinc-400 italic flex items-center gap-2">
                                                    <User size={12} className="text-emerald-400" /> {order.client}
                                                </span>
                                            </div>
                                            <h4 className="text-2xl font-black uppercase tracking-tighter italic leading-none group-hover:text-emerald-600 transition-colors uppercase italic truncate max-w-[500px]">{order.id} - {order.items}</h4>
                                            <div className="flex items-center gap-4 text-xs font-black text-zinc-400 uppercase italic opacity-60">
                                                <span>FULFILLMENT: {order.fulfillment}</span>
                                    ...
                                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-12 flex-1 w-full xl:w-auto px-10 border-x border-zinc-100 dark:border-zinc-800 border-dashed">
                                        <div className="flex flex-col items-center justify-center text-center space-y-2">
                                            ...
                                            <span className="text-2xl font-black italic text-zinc-900 dark:text-white uppercase">{order.amount}</span>
                                        </div>
                                        <div className="flex flex-col items-center justify-center text-center space-y-2">
                                            <span className="text-[10px] font-black uppercase italic text-zinc-400 tracking-widest">ETA Node</span>
                                            <span className="text-xl font-black italic text-zinc-900 dark:text-white uppercase">{order.delivery}</span>
                                        </div>
                                        <div className="flex flex-col items-center justify-center text-center space-y-2">
                                            <span className="text-[10px] font-black uppercase italic text-zinc-400 tracking-widest">Progress</span>
                                            <div className="flex items-center gap-2">
                                                <div className="h-1.5 w-12 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                                                    <div className={`h-full bg-emerald-500 ${order.status === 'Delivered' ? 'w-[100%]' : order.status === 'Shipped' ? 'w-[75%]' : 'w-[25%]'} transition-all`} />
                                                </div>
                                                <span className="text-xs font-black text-emerald-600 text-[8px]">{order.status === 'Delivered' ? '100%' : order.status === 'Shipped' ? '75%' : '25%'}</span>
                                            </div>
                                        </div>
                                        <div className="flex flex-col items-center justify-center text-center space-y-2">
                                            <span className="text-[10px] font-black uppercase italic text-zinc-400 tracking-widest">Integrity</span>
                                            <Badge className="bg-zinc-50 dark:bg-zinc-800 text-zinc-400 border border-zinc-200 px-4 py-1.5 font-black italic text-[8px] uppercase">VALIDATED</Badge>
                                        </div>
                                    </div>
                        ...
                        <div className="py-10 flex flex-col items-center justify-center space-y-6">
                            <CustomButton variant="ghost" className="h-20 px-20 rounded-full border-2 border-dashed border-zinc-100 text-zinc-300 font-black uppercase italic tracking-[0.4em] hover:bg-zinc-50 hover:text-emerald-600 hover:border-emerald-400 transition-all flex items-center gap-6 text-sm">
                                <Plus size={28} /> Synchronize Legacy Orders
                            </CustomButton>
                        </div>
                    </div>
                </div>

                {/* Automation & Warehouse row */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <Card className="rounded-[64px] p-16 border border-zinc-200 dark:border-zinc-800 shadow-2xl space-y-12">
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                <h3 className="text-3xl font-black uppercase tracking-tighter italic leading-none text-zinc-900 dark:text-white uppercase italic">Neural Fulfillment Node</h3>
                                <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest mt-2">Intelligent warehouse routing and stock reservation.</p>
                            </div>
                        </div>
                        <div className="space-y-8">
                            {[
                                { label: "Deterministic Stock Lock", desc: "Instantly reserve stock fragments upon quote acceptance.", status: "ENFORCED" },
                                { label: "Logistics Optimization Mesh", desc: "Route orders to the nearest fulfillment cluster automatically.", status: "ACTIVE" },
                            ].map((rule, i) => (
                    ...
                    <Card className="rounded-[64px] p-16 bg-zinc-900 border-0 shadow-3xl text-white relative overflow-hidden group">
                        <Truck className="absolute -bottom-20 -right-20 h-96 w-96 opacity-5 rotate-12 group-hover:rotate-0 transition-transform duration-[2000ms]" />
                        <div className="relative z-10 space-y-12">
                            <h3 className="text-5xl font-black tracking-tighter uppercase italic leading-none">Global Logistics Grid</h3>
                            <p className="text-xl font-bold text-zinc-400 italic uppercase tracking-[0.1em] leading-relaxed">Map your order fulfillment status across 124+ global corridors with real-time tracking and automated alerts.</p>
                            <div className="pt-10 flex flex-col gap-8">
                                <CustomButton className="bg-white text-zinc-900 rounded-[36px] h-20 w-full font-black uppercase tracking-widest shadow-2xl hover:scale-[1.02] transition-transform text-base italic flex items-center justify-center gap-4">
                                    Open Logistics Matrix <ArrowRight size={24} />
                                </CustomButton>
                            </div>
                        </div>
                    </Card>
