...
import { Shield, Plus, Search, MoreHorizontal, Users, Edit3, UserPlus, Lock, Trash2, ChevronRight, Filter, Loader2 } from "lucide-react"
...
export default function SecurityGroupsPage() {
    ...
    const fetchTeams = async () => {
        try {
            ...
            const mapped = teams
                ...
                .filter((g: Group) => g.type === "Security")
            setGroups(mapped)
        } catch (error: any) {
            toast.error(error?.response?.data?.message || "Failed to fetch security groups")
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleGroupCreated = async (group: GroupFormData) => {
        try {
            await createTeam({ ...group, type: "Security" })
            toast.success("Security group created successfully")
            fetchTeams()
        } catch (error: any) {
            toast.error(error?.response?.data?.message || "Failed to create security group")
        }
    }
    ...
    const handleGroupReleased = async (groupId: string) => {
        try {
            ...
            toast.error(error?.response?.data?.message || "Failed to delete security group")
        }
    }
    ...
    return (
        <div className="flex flex-col min-h-screen bg-transparent">
            <SubHeader
                ...
                rightControls={
                    <Button
                        className="h-8 gap-2 rounded-none bg-indigo-600 hover:bg-indigo-700 text-white font-bold uppercase tracking-widest text-[10px]"
                ...
                {loading ? (
                    ...
                    <Card className="rounded-none border-0 shadow-sm bg-white/50 backdrop-blur-sm">
                        ...
                        <CardContent className="h-64 flex items-center justify-center border-2 border-dashed border-zinc-200 m-6 rounded-none">
                            <div className="text-center text-zinc-500">
                                ...
                                <Button variant="link" className="text-indigo-600" onClick={() => setCreateGroupOpen(true)}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        {filteredGroups.map((group) => (
                            <Card key={group.id} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-none shadow-sm hover:shadow-md transition-all overflow-hidden border-t-3 border-t-indigo-500">
                                <CardHeader className="flex flex-row items-center justify-between pb-4 pt-5 px-5">
                                    <div className="h-10 w-10 bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-600 border border-indigo-200 dark:border-indigo-800 rounded-none font-semibold text-sm">
                                ...
                                <CardContent className="space-y-4 px-5 pb-5">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <Badge className="rounded-none border text-[10px] font-semibold py-0.5 px-2.5 bg-indigo-50 text-indigo-600 border-indigo-200">Security</Badge>
                        ...
                        <div
                            className="border-2 border-dashed border-zinc-200 dark:border-zinc-800 p-6 flex flex-col items-center justify-center text-center space-y-3 hover:border-indigo-300 transition-all cursor-pointer rounded-none"
                            onClick={() => setCreateGroupOpen(true)}
                        >
                            <div className="h-12 w-12 rounded-full bg-indigo-50 flex items-center justify-center">
                                <Plus className="w-5 h-5 text-indigo-400" />
                            </div>
                            <p className="text-xs font-semibold text-zinc-400">Add Security Group</p>
                        </div>
                    </div>
                )}
            </div>

            <CreateGroupModal open={createGroupOpen} onOpenChange={setCreateGroupOpen} defaultType="Security" onGroupCreated={handleGroupCreated} />
