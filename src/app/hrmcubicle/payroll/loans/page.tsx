        ...
        <div className="flex flex-col h-full bg-[#f8fafc] font-sans overflow-y-auto" style={{ zoom: "67%" }}>
            ...
            <div className="flex-1 p-8 space-y-6 pb-32">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    {[
                        { label: "Active Loans", val: stats.activeCount, icon: CreditCard, color: "text-[#8B5CF6]", bg: "bg-[#8B5CF6]/10" },
                        { label: "Total Outstanding", val: `₹${(stats.totalOutstanding / 100000).toFixed(1)}L`, icon: DollarSign, color: "text-amber-500", bg: "bg-amber-50" },
                        { label: "EMIs Due This Month", val: stats.emisDue, icon: Clock, color: "text-blue-500", bg: "bg-blue-50" },
                        { label: "Avg Interest Rate", val: `${stats.avgRate}%`, icon: Percent, color: "text-emerald-500", bg: "bg-emerald-50" },
                    ].map((s, i) => (
