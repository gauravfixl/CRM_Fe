...
export default function CreateOrganizationPage() {
  ...
  return (
    <div className="create-org-page min-h-screen flex items-center justify-center bg-[#1e293b] p-4">
      <div className="create-org-wrapper w-full max-w-5xl">
        <Card className="create-org-card shadow-2xl border-0 overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-[300px_1fr]">
            {/* Left Panel - Branding */}
            <div className="create-org-side bg-gradient-to-br from-blue-600 to-blue-800 p-8 flex flex-col items-center justify-center text-white">
              <div className="create-org-logo-wrapper flex justify-center mb-6">
                <div className="create-org-logo relative w-28 h-28">
                  <Avatar className="create-org-avatar w-28 h-28 mx-auto border-4 border-white shadow-lg">
                    {avatarPreview ? (
                      <AvatarImage
                        src={avatarPreview}
                        alt="org logo"
                      ...
                      <AvatarFallback className="create-org-avatar-fallback bg-blue-500 text-white text-lg">
                        <Building2 className="create-org-icon w-8 h-8" />
                      </AvatarFallback>
                    )}
                  </Avatar>
                  <input
              ...
              <div className="create-org-header-text text-center">
                <CardTitle className="create-org-title text-2xl font-bold text-white">
                  Create Your Organization
                </CardTitle>
                <CardDescription className="create-org-subtitle text-blue-100 mt-2">
                  Set up your organization to continue
                </CardDescription>
                <p className="text-xs text-blue-200 mt-4">Click the logo to upload</p>
              </div>
            </div>

            {/* Right Panel - Form */}
            <CardContent className="create-org-content p-6 md:p-8">
              <form onSubmit={handleCreateOrg} className="create-org-form space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Organization Name */}
                  <div className="create-org-field space-y-1.5 md:col-span-2">
                    ...
                    <Input
                      ...
                      className={`create-org-input h-10 ${getInputBorderClass(formData.orgName, errors.orgName, touchedFields.orgName)}`}
                    />
                    {touchedFields.orgName && !errors.orgName && formData.orgName.trim() ? (
                      ...
                      <p className="text-[10px] text-gray-400">Numbers are not allowed</p>
                    )}
                  </div>

                  {/* Contact Name */}
                  <div className="create-org-field space-y-1.5">
                    ...
                    <Input
                      ...
                      className={`create-org-input h-10 ${getInputBorderClass(formData.contactName, errors.contactName, touchedFields.contactName)}`}
                    />
                    {touchedFields.contactName && !errors.contactName && formData.contactName.trim() ? (
                      ...
                      <p className="text-[10px] text-gray-400">Only alphabets allowed</p>
                    )}
                  </div>
                  ...
                  <div className="create-org-field space-y-1.5">
                    ...
                    <Input
                      ...
                      readOnly
                      className={`create-org-input h-10 cursor-not-allowed bg-gray-50 ${getInputBorderClass(formData.contactEmail, errors.contactEmail, touchedFields.contactEmail)}`}
                    />
                    {touchedFields.contactEmail && !errors.contactEmail && formData.contactEmail.trim() ? (
                      <p className="text-xs text-green-600">Pre-filled from your account</p>
                    ) : (
                  ...
                  <div className="create-org-field space-y-1.5">
                    ...
                    <Input
                      ...
                      readOnly
                      className={`create-org-input h-10 cursor-not-allowed bg-gray-50 ${getInputBorderClass(formData.contactPhone, errors.contactPhone, touchedFields.contactPhone)}`}
                    />
                  ...
                  <div className="create-org-field space-y-1.5">
                    ...
                    <Input
                      ...
                      className={`create-org-input h-10 ${getInputBorderClass(formData.address, errors.address, touchedFields.address)}`}
                    />
                    {touchedFields.address && !errors.address && formData.address.trim() ? (
                      ...
                      <p className="text-[10px] text-gray-400">Min 10 chars; letters, numbers, , . : - allowed</p>
                    )}
                  </div>
                  ...
                  <div className="md:col-span-2">
                    <CountryStateCityDropdown
                      ...
                      cityName="orgCity"
                    />
                    {(errors.orgCountry || errors.orgState || errors.orgCity) && (
                      <div className="mt-1 space-y-0.5">
                        {errors.orgCountry && (
                          <p className="create-org-error text-red-500 text-xs">{errors.orgCountry}</p>
                        )}
                        {errors.orgState && (
                          <p className="create-org-error text-red-500 text-xs">{errors.orgState}</p>
                        )}
                        {errors.orgCity && (
                          <p className="create-org-error text-red-500 text-xs">{errors.orgCity}</p>
                        )}
                      </div>
                    )}
                  </div>
                ...
                <Button
                  type="submit"
                  className="create-org-submit w-full h-11 bg-blue-600 hover:bg-blue-700 mt-2"
                  disabled={isLoading}
                >
                  {isLoading ? "Creating Organization..." : "Create Organization"}
                </Button>

                <div className="create-org-footer text-center pt-1">
