...
import { getBoardById, type BoardColumn } from "@/modules/project-management/board/hooks/boardHooks"
import { addTask, getTaskById, getTasksByColumn, deleteTask, reorderTask, type Task } from "@/modules/project-management/task/hooks/taskHooks"
...
interface Column extends BoardColumn {
  id?: string
  tasks?: Task[]
}
...
export function ProjectBoard({ projectId, project, teams, tasks, setTasks }: ProjectBoardProps) {
  ...
  const [tasksByColumn, setTasksByColumn] = useState<Column[]>([])
  ...
  const [newTaskType, setNewTaskType] = useState<"task" | "bug">("task")
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    try {
      await reorderTask(projectId, draggableId, {
        boardId,
        from: filteredTasks.find(c => c.id === source.droppableId)?.order?.toString() || "",
        to: toOrder?.toString() || "",
      })
      setTasks(tasks.map((t: Task) => t._id === draggableId ? { ...t, columnOrder: toOrder || 0 } : t))
    } catch (error) {
      console.error("Error reordering task:", error)
    }
  }

  const columnsArray = (Array.isArray(tasksByColumn) ? tasksByColumn : Object.values(tasksByColumn)) as Column[]
  const filteredTasks = columnsArray.map((col: Column) => {
    const term = searchTerm?.toLowerCase() || ""
    const columnTasks = Array.isArray(col.tasks) ? col.tasks : []
    const filteredColumnTasks = columnTasks.filter((task: Task) =>
  ...
  const handleUpdateColumn = async () => {
    ...
    try {
      ...
      setTasksByColumn((prev: Column[]) => prev.map(col => (col._id || col.id) === columnId ? { ...col, name: editColumnTitle } : col))
  ...
  const handleSaveColumn = async () => {
    ...
    try {
      ...
      setTasksByColumn((prev: Column[]) => [...prev, { ...response.data.column, tasks: [] }])
      setNewColumnTitle("")
  ...
  const handleDeleteTaskConfirm = async (taskId: string | null) => {
    if (!taskId) return
    try {
      await deleteTask(projectId, taskId || "", { boardId })
      setTasksByColumn((prev: Column[]) => prev.map(col => ({
        ...col,
        tasks: col.tasks?.filter((t: Task) => t._id !== taskId)
      })))
    } catch (error) {
    ...
    <div className="space-y-6 h-[100vh] overflow-x-hidden">
              ...
              <Droppable droppableId={column.id || column._id}>
                {(provided, snapshot) => (
                  <div
                    ...
                    className={`flex-1 p-3 space-y-2 overflow-y-auto ${snapshot.isDraggingOver ? "bg-slate-200/50 dark:bg-slate-800/50" : ""}`}
                  >
                        ...
                        <Draggable key={task._id} draggableId={task._id} index={index}>
                          {(provided, snapshot) => (
                            <SmallCard
                              ...
                              className={`mb-3 cursor-pointer border-none shadow-sm transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 bg-white dark:bg-slate-900 ${snapshot.isDragging ? "shadow-xl ring-2 ring-primary/20 rotate-1" : ""
                                  ...
                                  <div className="flex items-center justify-between mt-1">
                                    <div className="flex items-center gap-2">
                                      <div className="flex items-center gap-1 group">
                                        <TypeIcon className={`h-3.5 w-3.5 ${task.type === 'bug' ? 'text-red-500' : 'text-blue-500'}`} />
                                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter tabular-nums">
                                          {task.taskCode || task._id.slice(-4)}
                                        </span>
                                      </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2">
                                      <Avatar className="h-6 w-6 ring-2 ring-white dark:ring-slate-800">
                                        <AvatarImage src={task.assigneeId?.userId?.avatar} />
                                        <AvatarFallback className="text-[10px] bg-slate-100 text-slate-600">
                                          {task.assigneeId?.userId?.fullName?.charAt(0) || "U"}
                                        </AvatarFallback>
                                      </Avatar>
      ...
      {selectedTask && selectedTaskData && (
        <TaskDialog
          ...
          tasks={filteredTasks}
          setTasks={setTasks}
          filterData={{ assignee: members.map(m => ({ id: { email: m.userId.email }, text: { firstName: m.userId.fullName } })) }}
          selectedTaskData={selectedTaskData}
          onTaskUpdate={(updated: Task) => setTasks(tasks.map((t: Task) => t._id === updated._id ? updated : t))}
        />
      )}
      ...
      <CustomDialog open={isAddTaskOpen} onOpenChange={setIsAddTaskOpen}>
        <CustomDialogContent>
          ...
          <div className="space-y-4">
            ...
            <CustomSelect value={newTaskPriority} onValueChange={(val) => setNewTaskPriority(val as "low" | "medium" | "high" | "critical")}>
