  ...
  return (
    <section id="security" className="bg-[#F5F5F5] py-20">
      <div ref={ref as any} className="max-w-[1280px] mx-auto px-6">
        ...
        >
          <span className="text-[#0067B8] text-xs uppercase tracking-widest font-semibold">
            Security & Compliance
          </span>
          <h2 className="mt-3 text-[32px] lg:text-[40px] font-semibold text-[#1A1A1A] leading-tight">
            Enterprise-grade security you can trust
          </h2>
          <p className="mt-4 text-[#505050] text-lg max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {badges.map((badge, i) => (
            <motion.div
              key={badge.title}
              className="bg-white rounded-lg p-8 border border-[#E5E5E5] hover:shadow-md transition-shadow duration-300"
              initial={{ opacity: 0, y: 24 }}
              animate={isVisible ? { opacity: 1, y: 0 } : {}}
        ...
        >
          {stats.map((stat) => (
            <div key={stat.label} className="text-center">
              <p className="text-[36px] lg:text-[42px] font-bold text-[#0067B8]">
                {stat.value}
              </p>
              <p className="text-[#505050] text-sm mt-1 font-medium">
                {stat.label}
              </p>
            </div>
          ))}
        ...
        <motion.div
          className="flex items-center justify-center gap-3 text-[#6B6B6B] text-sm"
          initial={{ opacity: 0 }}
          animate={isVisible ? { opacity: 1 } : {}}
          transition={{ duration: 0.6, delay: 0.7 }}
        >
          <ShieldCheck size={18} className="text-[#107C10] shrink-0" />
          <span>
