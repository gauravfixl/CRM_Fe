...
interface EditSecondaryDetailsWizardProps {
  isOpen: boolean;
  onClose: () => void;
  fields: Record<string, string>; // from parent (secondaryDetails)
  onSave: (updated: Record<string, string>) => void;
}

export default function EditSecondaryDetailsWizard({
  isOpen,
  onClose,
  fields,
  onSave,
}: EditSecondaryDetailsWizardProps) {
  ...
  const handleSave = () => {
    ...
    localFields.forEach((f) => {
      if (f.name.trim()) updated[f.name] = fields[f.name] || "N/A";
    });
    onSave(updated);
    onClose();
  };
